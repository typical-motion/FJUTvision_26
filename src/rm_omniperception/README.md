...
| [decider.cpp](src/omniperception/decider.cpp) | 决策逻辑：`armor_filter` 过滤 → 优先级排序 → `sort` 跨相机排序 → 角度换算 |
...
参考 sp_vision25 的优先级系统，按装甲板类型分配权重（`getPriority()` + `std::sort`）。
编号映射：1=英雄、2=工程、3/4/5=步兵。

**模式 1 — 英雄优先** (`priority_mode: 1`)：
```
步兵3/4 > 英雄1 > 步兵5/哨兵 > 工程2 > 前哨站/基地
```

**模式 2 — 步兵优先** (`priority_mode: 2`)：
```
工程2 > 英雄1/步兵3/4/5 > 哨兵/前哨站/基地
```

#### 3.3 跨相机排序 (`sort`)

`popLatestDetection()` 取出队列中所有待处理结果：
1. 对每个 `DetectionResult` 内部先 `armor_filter` 再按优先级排序
